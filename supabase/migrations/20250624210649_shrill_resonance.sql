...
-- STEP 1: Insert default lead sources (auto-assigns first available tenant_id)
INSERT INTO lead_sources (tenant_id, name, type, tracking_code, conversion_rate)
SELECT t.id, s.name, s.type, s.tracking_code, s.conversion_rate
FROM (
  VALUES 
...
) AS s(name, type, tracking_code, conversion_rate),
(SELECT id FROM tenants LIMIT 1) t
ON CONFLICT DO NOTHING;

-- STEP 2: Insert sample sales representatives
INSERT INTO sales_reps (name, email, phone, territory, monthly_target, quarterly_target, annual_target)
VALUES
...
-- STEP 3: Insert sample vehicles
INSERT INTO vehicles (
  vin, make, model, year, type, price, cost, location, features, images
)
VALUES
