...
function PortalDashboard() {
  ...
  useEffect(() => {
    ...
    if (isPreview && previewClientId) {
      ...
      if (clientAccount) {
        ...
        setActiveClient(fallback);
        saveToLocalStorage('renter-insight-client-session', fallback);
      }
    } else {
  ...
  return (
    <div className="p-6">
      {/* Admin dashboard content (users, actions, etc.) goes here */}
      {/* You can build out the full portalUsers listing and user interaction components here */}
    </div>
  );
}
