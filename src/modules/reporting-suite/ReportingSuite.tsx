...
import { ReportGeneratorForm, ReportConfig } from './components/ReportGeneratorForm'
...
function ReportingDashboard() {
  ...
  const handleGenerateReport = (config: ReportConfig | { id: string; name: string; type: ReportType; description: string; icon: any; color: string }) => {
    console.log('ReportingSuite: handleGenerateReport called with config:', config); // Debug log

    let reportConfigToGenerate: ReportConfig;

    // Check if the config is a template object (e.g., by checking for 'id' property)
    if ('id' in config && 'description' in config) { // It's a template object
      ...
      reportConfigToGenerate = {
        type: config.type,
        name: config.name,
        dateRange: {
          startDate: defaultStartDate.toISOString().split('T')[0], // Format as YYYY-MM-DD string
          endDate: defaultEndDate.toISOString().split('T')[0] // Format as YYYY-MM-DD string
        },
        filters: {} // Templates typically start with no filters
      };
    } else { // It's a ReportConfig object from the form
      reportConfigToGenerate = config as ReportConfig;
    }
    generateReport(reportConfigToGenerate);
  };
